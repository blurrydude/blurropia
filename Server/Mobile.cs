	...
	public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable, IDamageable
	{
		...
		*/
		...
		public virtual void OnDeath(Container c)
		{
		    ...
		    var killedBy = LastKiller == null ? "an unknown entity or event" : LastKiller.Name;

			if (!m_Player)
			{
				Delete();
                
			    //World.Broadcast(0x35, true, killedBy + " killed " + Name);
			    //DBLogger.LogEvent(LastKiller, "killed " + Name);
			}
			else
			{
		...
		{
            //SaveUtility.SaveMobileToDatabase(this);
