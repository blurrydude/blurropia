...
namespace Server.Misc
{
    public class AutoSave : Timer
    {
        ...
        {
            //AlternativePersistence.SaveMobiles(World.Mobiles);
            /*foreach (var mob in World.Mobiles)
            {
                ScriptUtility.SaveMobileToDatabase(mob.Value);
            }

            foreach (var item in World.Items)
            {
                ScriptUtility.SaveItemToDatabase(item.Value);
            }*/
            AutoSave.Save(false);
        }
