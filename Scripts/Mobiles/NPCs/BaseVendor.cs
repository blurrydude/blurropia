...
namespace Server.Mobiles
{
	...
	public abstract class BaseVendor : BaseCreature, IVendor
	{
		...
		{
		    //LoadSBInfo();
		    /*foreach (var sbinfo in SBInfos)
		    {
		        foreach (var gbi in sbinfo.BuyInfo)
		        {
		            var typePath = ScriptUtility.TypePath(gbi.Type);
		            var typeReal = typePath?.Split('.').Last();
		            gbi.Price = DatabaseUtility.GetMarketData(typeReal, typePath, gbi.Price, 1)?.CurrentPrice ?? gbi.Price;
		        }
		    }*/
